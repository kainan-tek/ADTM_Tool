...
from PySide6.QtGui import QIcon, QIntValidator, QCloseEvent
...
class MainWindow(QMainWindow):
    ...
    def init_var(self):
        ...
        self.alsanode_pattern = re.compile(
            r"alsa node\((.*)\) adtm"
        )  # alsa node(pcmMicRefIn_c) adtm
    ...
    def init_gui(self):
        ...
        self.ui.guideTextEdit.setStyleSheet("background-color: rgb(231, 234, 237);")
    ...
    def call_select_bt(self):
        ...
        try:
            ...
            self.msgbox.critical(
                self, "Error", msgtext
            )  # warning, information, about, question
            return False
    ...
    def call_draw_bt(self):
        ...
        draw_thread = threading.Thread(
            target=self.drawing_thread, daemon=True, name="Draw Thread"
        )
        draw_thread.start()
    ...
    def drawing_thread(self):
        ...
        try:
            with open(
                self.draw_dict["log_file"], mode="r", encoding="utf-8", errors="ignore"
            ) as fp:
                all_log_list = fp.readlines()
        except Exception:
            msgtext = "Error of opening log file"
        ...
        target_log_list = [
            item
            for item in all_log_list
            if re.search(self.draw_dict["alsa_node"], item, re.I)
            and self.key_words[0] in item
        ]
        ...
        if not target_log_list:
            ...
            return False
        actual_time_list = [
            int(self.max_pattern.findall(item)[0]) for item in target_log_list
        ]
        ...
        fig.update_layout(
            title=self.draw_dict["alsa_node"],
            xaxis_title="unit: second",
            yaxis_title="unit: ms",
            legend_font_size=16,
        )
